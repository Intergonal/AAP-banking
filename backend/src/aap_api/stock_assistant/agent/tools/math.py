...
def calculate_returns(prices: str) -> str:
    """Use when the user asks about returns, gain/loss percentages, or drawdown on a price series. prices is a JSON array of numbers (e.g. from get_price_series). Returns total return, mean and std of per-period returns, latest return, and max drawdown."""
    try:
...
def moving_average(prices: str, window: int = 20) -> str:
    """Use when the user asks for a moving average (SMA) or whether price is above/below its average. prices is a JSON array of numbers; window is the number of periods (default 20). Returns the latest SMA, whether price is above or below it, and a preview of the SMA series."""
    try:
        arr = _as_array(prices)
...
def volatility(prices: str, window: int = 20) -> str:
    """Use when the user asks about volatility or standard deviation of a price series. prices is a JSON array of numbers; window is the rolling window in periods (default 20)."""
    try:
        arr = _as_array(prices)
...
def correlation(tickers: str, period: str = "1mo") -> str:
    """Use when the user asks whether stocks move together or for the correlation between tickers. tickers is a JSON array of 2-8 ticker symbols (e.g. [\"AAPL\", \"MSFT\"]); period options: 5d, 1mo, 3mo, 6mo, 1y, 5y. Fetches price series from Yahoo Finance."""
    try:
...
def portfolio_stats(returns_arrays: str, weights: str, rf: float = 0.0) -> str:
    """Use when the user asks for portfolio expected return, volatility, or Sharpe ratio from asset return series. returns_arrays is a JSON array where each element is a JSON array of per-period returns for one asset (e.g. [[0.01, -0.005, 0.002], [0.003, ...]]); weights is a JSON array of asset weights summing to 1 (e.g. [0.5, 0.5]); rf is the risk-free rate per period (default 0.0)."""
    try:
...
def cagr(start_value: float, end_value: float, years: float) -> str:
    """Use when the user asks for compound annual growth rate (CAGR) or average annual growth. start_value: beginning amount; end_value: ending amount; years: number of years. Returns CAGR as a percentage and total growth."""
    if start_value <= 0:
...
def linear_trend(prices: str) -> str:
    """Use when the user asks about a price trend line, slope, or linear regression fit. prices is a JSON array of numbers. Returns the slope per period, the approximate trend over the whole window, the intercept, and the R-squared coefficient."""
    try:
        arr = _as_array(prices)
        if arr.size < 3:
